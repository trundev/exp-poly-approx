...
def select_exponent_bases(poly_arr: PolyArray) -> tuple[DataArray, IndexArray]:
...
def interpolate_number_wall(data: DataArray, *, simplify: bool=False) -> sympy.Expr:
    ...
    """
    # Fid characteristic polynomial, its roots are the exponent bases
    char_poly_arr = char_poly_number_wall(data, sympy.abc.x)
    print(f'  Characteristic polynomial(s): {char_poly_arr}')
    bases, repeats = select_exponent_bases(char_poly_arr)
